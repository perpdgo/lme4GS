...
#g(y|u)=X*beta + Z_1 u_1 + Z_2 u_2 + ... + Z_q u_q
#u = c(u_1,...,u_q)
#NA are not allowed in y

glmerUvcov <- function(formula, data = NULL, Uvcov = NULL,  family = NULL, verbose = 0L)
{
  	...
  	{
  		stop("Use 'lmerUvcov()' when calling setting the family to 'gaussian' and 'identity' link\n")
  	}
	...
	parsedFormula$reTrms[["Ztlist"]] <- Ztlist
  	...
  	devianceFunction<-do.call(mkGlmerDevfun,
                            c(parsedFormula,
                              list(start=NULL,verbose=verbose,control=control)))
  
  	#3 The optimization module
  	optimizerOutput<-optimizeGlmer(devianceFunction, optimizer="Nelder_Mead",
  	                               verbose=verbose)
